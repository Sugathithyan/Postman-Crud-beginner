...
db = client.get_database('fir_db')

user_collection = pymongo.collection.Collection(db, 'Datab1')
...
def project_api_routes(endpoints):
    ...
    def read_users():
        ...
        try:
            ...
            output = [{'Name' : user['name'], 'Email' : user['age']} for user in users]   #list comprehension
            resp['data'] = output
        except Exception as e:
            print(e)
            status = {
